"""Natural-reasoning attribution pipeline for Qwen2.5-3B.

No few-shot prompting: players are paragraph-level chunks of the model's own
CoT output — no step format is imposed, so the reasoning is genuinely the
model's own. build_prompt_natural joins chunks on double newlines (paragraph
boundaries), parse_natural_steps splits on blank lines (falling back to
sentences), and the last paragraph is treated as the target/conclusion.
"""
...
def load_model(model_id: str = "Qwen/Qwen2.5-3B-Instruct") -> tuple:
    """Load the causal LM + tokenizer used for CoT generation and scoring."""
